...
import { Iterator, pathParse, saveAsFile, merge, getRelativePath } from './js/utils.js';
import { queryBlock, queryAsset, getNotebookConf, exportMdContent, updateBlock, getFile, putFile, resolveAssetPath } from './js/api.js';
...
async function init(params) {
	...
	switch (params.mode) {
		case 'inbox': // 收集箱
			...
			if (r) {
				...
				try {
					...
					params.breadcrumb.set(typeText, hpathText, typeTitle, hpathTitle, blockHref, docHref); // 设置面包屑
				}
			}
			break;
		case 'assets': // 资源文件
			switch (true) {
				...
				case params.path.startsWith('https://'):
					...
					if (r) {
						...
						params.breadcrumb.set(`${config.editor.mark.url}${config.editor.MAP.LABELS.mode[params.mode][params.lang] || config.editor.MAP.LABELS.mode[params.mode].default}`, `${config.editor.mark.urlpath}${url.host}${url.pathname}`.replaceAll('/', config.editor.mark.pathseparate), filename, params.url, params.url, params.url); // 设置面包屑
		...
		case 'local': // 本地文件
			...
			// 文件路径
			let hpathText = params.url.startsWith(params.workspace) ? params.path.substring(1) : params.url;
			r = await getFile(params.path); // 获取文件内容
			if (r) {
				params.value = await r.text(); // 文件内容
				...
				params.ext = ext;
				if (params.language === 'default' && ext) params.language = ext; // 如果没有设置语言, 则根据文件扩展名设置语言
				params.breadcrumb.set(`${config.editor.mark.file}${config.editor.MAP.LABELS.mode[params.mode][params.lang] || config.editor.MAP.LABELS.mode[params.mode].default}`, `${config.editor.mark.filepath}${hpathText}`.replaceAll('/', config.editor.mark.pathseparate), filename, params.url, config.editor.link.file(params.url), config.editor.link.directory(params.dir)); // 设置面包屑
			} else {
				params.mode = 'none';
				return;
			}
		...
		case 'block': // 块
			...
			// console.log(params);
			params.block = b;
			params.breadcrumb.set(`${config.editor.mark.block}${config.editor.MAP.LABELS.type[b.type][params.lang] || config.editor.MAP.LABELS.type[b.type].default}`, `${config.editor.mark.blockpath}${n.name}${b.hpath.replaceAll('/', config.editor.mark.pathseparate)}`, `siyuan://blocks/${b.id}`, `${n.name}${b.hpath}`, `siyuan://blocks/${b.id}`, `siyuan://blocks/${b.root_id}`); // 设置面包屑
			break;
		case 'none':
		default:
			break;
...
window.onload = () => {
	...
	window.editor.params = {
		 ...
		 *     -> 'doc': 文档块
		 */
		mode: window.editor.url.searchParams.get('mode') || 'none', // 编辑器模式
		value: '', // 内容
		theme: window.editor.url.searchParams.get('theme') || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 1 : 0), // 主题模式
		lang: window.editor.url.searchParams.get('lang') || 'default', // 语言
		language: window.editor.url.searchParams.get('language') || 'default', // 语言模式
		tabSize: parseInt(window.editor.url.searchParams.get('tabSize')) || 4, // 缩进空格数量
		workspace: (window.editor.url.searchParams.get('workspace') || '').replaceAll(/(\\|\/)+/g, '/'), // 工作空间路径
	...
	init(window.editor.params).then(() => {
		...
		require.config({
			'vs/nls': {
				availableLanguages: {
					'*': config.editor.MAP.LANGS[window.editor.params.lang] || config.editor.MAP.LANGS.default || '',
				},
			},
		});

		require(['vs/editor/editor.main'], () => {
			const language = config.editor.MAP.LANGUAGES[window.editor.params.language.toLowerCase()] || window.editor.params.language || 'plaintext';
			...
			const options = merge(
				...
				config.editor.IStandaloneEditorConstructionOptions, // 默认配置
				{
					...
					theme: config.editor.MAP.THEMES[window.editor.params.theme] || config.editor.MAP.THEMES.default || 'vs', // 主题
				...
				window.editor.params.body ? window.editor.params.body.IStandaloneEditorConstructionOptions || {} : {} // URL hash 配置
			);
			// console.log(options);

			window.editor.editor = monaco.editor.create(container, options);
			async function save() {
				// 保存文件
				let response;
				switch (window.editor.params.mode) {
					...
					case 'inbox':
						...
						break;
					case 'local':
						response = await putFile(window.editor.params.path, window.editor.editor.getValue()).then(() => config.editor.command.SAVED());
						break;
					case 'assets':
						response = await putFile(window.editor.params.path, window.editor.editor.getValue());
						break;
					case 'query':
						response = await updateBlock(window.editor.params.id, `\{\{${window.editor.editor.getValue().trim()}\}\}\n${window.editor.params.block.ial}`);
						break;
					case 'code':
						response = await updateBlock(window.editor.params.id, `\`\`\`${window.editor.params.language}\n${window.editor.editor.getValue()}\n\`\`\`\n${window.editor.params.block.ial}`);
						break;
					case 'doc':
						response = await updateBlock(window.editor.params.id, window.editor.editor.getValue());
						break;
					case 'html':
					case 'block':
						response = await updateBlock(window.editor.params.id, `${window.editor.editor.getValue().trim()}\n${window.editor.params.block.ial}`);
			...
			window.editor.editor.addAction({
				...
				label: config.editor.MAP.LABELS.wrap[window.editor.params.lang] || config.editor.MAP.LABELS.wrap.default, // 菜单项名称
			...
			window.editor.editor.addAction({
				...
				label: config.editor.MAP.LABELS.save[window.editor.params.lang] || config.editor.MAP.LABELS.save.default, // 菜单项名称
			...
			window.editor.editor.addAction({
				...
				label: config.editor.MAP.LABELS.saveAs[window.editor.params.lang] || config.editor.MAP.LABELS.saveAs.default, // 菜单项名称
			...
			if (window.editor.params.mode === 'assets' || window.editor.params.mode === 'local') {
				window.editor.editor.addAction({
					...
					label: config.editor.MAP.LABELS.openFileInVscode[window.editor.params.lang] || config.editor.MAP.LABELS.openFileInVscode.default, // 菜单项名称
				...
				window.editor.editor.addAction({
					...
					label: config.editor.MAP.LABELS.openDirInVscode[window.editor.params.lang] || config.editor.MAP.LABELS.openDirInVscode.default, // 菜单项名称
			...
			window.editor.editor.addAction({
				...
				label: config.editor.MAP.LABELS.copyhref[window.editor.params.lang] || config.editor.MAP.LABELS.copyhref.default, // 菜单项名称
				keybindings: [monaco.KeyMod.CtrlCmd | monaco.KeyMod.Alt | monaco.KeyCode.KeyC], // 绑定快捷键
				contextMenuGroupId: '9_window', // 所属菜单的分组
				contextMenuOrder: 1, // 菜单分组内排序
				run: () => {
					window.navigator.clipboard.writeText([window.location.pathname, window.location.search, window.location.hash].join(''));
				}, // 点击后执行的操作
			});
			...
			window.editor.editor.addAction({
				...
				label: config.editor.MAP.LABELS.copyfullhref[window.editor.params.lang] || config.editor.MAP.LABELS.copyfullhref.default, // 菜单项名称
